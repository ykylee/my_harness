...
- 상태: stable
- 최종 수정일: 2026-06-10 (D-65 TASK-005-2 v1.5 종료 선언, **5 사이클 + D-62 + 14 신규 test / 0 fail**. W17 (D-60) + W18 (D-61) + D-62 (W17 누락분 정정) + W19-1 (D-62 AuthStore 주입) + W20 (D-63 Ollama cascade) + W21 (D-64 sha256_8 + sort fix) 모두 완료. **v1.5 build phase 완전 종료**. ONNX 통합 v2.0 Planning 으로 연기. 424 workspace tests / 0 fail, binary 13MB 유지)
...
- [ ] **TASK-005-2 v2.0 Planning** — yklee 결정 시. 후보: ONNX 3-commit (ort + CCR semantic + Kompress ONNX) / Plugin 4-계층 (큰 사이클) / Kompress-back (low priority)
