...
- 상태: stable
- 최종 수정일: 2026-06-10 (D-62 + D-62 W19-1 + D-63 W20 + **D-64 W21**, **W17 누락분 = 0건 정정** + **TC-W17-002 test fail 복구 (AuthStore 주입)** + **F-3 Ollama native cascade** + **F-1+F-2 통합 (sha256_8 + sort fix)**. W17 original 4 commit 모두 main 머지 확인. W19-1 = store 주입. W20 = `probe_local_models` 2-stage cascade. W21 = hash8 (sha256 8-char) + backup filename `<ts>.<hash8>` + cleanup_old_backups sort bug fix. **누적 4 L1 + 3 L2 = 7 신규 test (W19-1) + 3 L2 (W20) + 4 L1 (W21) = 14 신규**, 130 llm tests / 0 fail)
...
- [ ] **TASK-005-2 v1.5 종료 선언 또는 Plugin 4-계층 / CCR (v1.5 후속)** — yklee 결정
