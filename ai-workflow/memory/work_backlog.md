...
- 상태: stable
- 최종 수정일: 2026-06-11 (**D-74 LLM Wiki 정상화** — lint 0/98/0 → 0/0/0 (pages=72). 3 sub-task: L08 fix (index.md full path) + L03 skip patch (run_wiki_lint.py 5 line) + 3 project skip config 보강. R-4 SSOT drift 정정 (DETAILED_DESIGN_LLM_WIKI.md 6 곳 path 갱신). wiki commit bd9b108 Gitea + my_harness commit 5407f82 dual push. 29/29 wiki-lint tests pass. 이전: D-73 T-v5-sync-1 plan_3c8c4a49 cancel + 4-plan split)
