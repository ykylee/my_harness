...
- [x] **TASK-005-1 v1 MVP 종료 선언** (8/8 waves + D-52 follow-up 6 작업 완료, D-59 follow-up)
...
| **D-60** | **TASK-005-2 v1.5 W17 완료 (`myharness auth add-local` 비대화형 모드)** — DD-AddLocal §6.3 OI-1 해소 (v1.5 첫 작업). clap flag 4개 (`--url/--model/--token/--probe-skip`). `myharness-llm::register_local_provider_non_interactive(url, token, model_id)` probe 스킵 helper. `handle_auth_add_local` 3-mode 분기 (non-interactive / interactive / partial flag error). `print_register_report` 공통 출력. CI/스크립트 환경에서 stdin/stdout non-tty 라도 사용 가능. 4 L1 + 2 L2 TC = 6/6 PASS, 410 workspace tests, 4 commit dual-push (feature/v15-add-local-non-interactive, Gitea + GitHub, PR 미작성 yklee 결정). **R-4 (사용자 home providers.toml 덮어쓰기)**: manual test 중 yklee 의 LM Studio 설정 1회 덮어쓰기 → mavis-trash recovery, F-1 backup / F-2 --yes flag v1.5+ OOS | §5.2 (12 명령어) + §5.5 + §6.3 (OI-1 ✅) |
